...
lapply(1:nrow(accounts_show), function(i){
  ...
  output[[curr_acct_id]] <- renderUI({
    ...
    temp <- df %>%
      ...
      dplyr::mutate(recurring = FALSE, property = 'n/a', data_table = dattbl_show$name[1]) %>% 
                ...
                column(
                  ...
                  tags$div(
                    ...
                    tagList(
                      ...
                      tags$div(class = "task_div", selectInput(paste0("fp_ipt_trn_",curr_acct_id,"_ppty",k), label = "property", choices = c('n/a', property_show$name), selected = ppty_k, width = entry_wid_m)),
                      ...
                      tags$div(class = "task_div", checkboxInput(paste0("fp_ipt_trn_",curr_acct_id,"_recu",k), label = "recurring", value = recu_k, width = entry_wid_s))
                      # add more tag here
                    )
